...
dateUtil = {
    ...
    /**
     * 日期格式化
     * val: date或yyyy-MM-dd HH:mm:ss
     */
    dateFormat_yyyy_MM_dd : function (val, pattern) {
        pattern = pattern === undefined ? "yyyy-MM-dd" : pattern;
        if (val && typeof val=="string") {
            return val.substring(0,10);
        }
        return dateUtil.format(val, pattern);
    },
    dateFormat_yyyy_MM_dd_hh_mm : function (val, pattern) {
